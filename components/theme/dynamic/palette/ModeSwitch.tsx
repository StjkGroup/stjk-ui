...
const ModeSwitch = (props: SwitchProps) => {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setChecked(_checked);
    dispatchEvent(_checked ? 'dark' : 'light');
  };

  useEffect(() => {
    if(theme.palette.mode === 'dark' && !checked){
      setChecked(true);
    }
    if(theme.palette.mode === 'light' && checked){
      setChecked(false);
    }
  }, [theme.palette.mode]);

  const dispatchEvent = (mode: PaletteMode) => {
    setTimeout(() => {
